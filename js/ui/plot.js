...
export function exportCurrentCurvePNG(filename) {
  ...
  if (!svgEl) return;
  exportSVGAsPNG(svgEl, filename || 'enrichment_curve.png');
}

export async function exportAllCurves(results, pathways, geneNames, width = 900) {
  const offscreen = document.createElement('div');
  offscreen.style.cssText =
    'position:fixed;left:-9999px;top:0;width:' + width +
    'px;visibility:hidden;pointer-events:none';
  document.body.appendChild(offscreen);

  for (let i = 0; i < results.length; i++) {
    const r    = results[i];
    const zoom = { i0: 0, i1: 1 };
    _renderInto(r, pathways, geneNames, offscreen, width, zoom);
...
export function exportSVGAsPNG(svgEl, filename) {
  return _svgToPNGAsync(svgEl, filename || 'chart.png').catch(console.error);
}
...
function _render() {
  ...
  _renderInto(result, pathways, geneNames, container, W, _zoom);
  _attachInteraction(container, result, pathways, geneNames);
}
...
function _renderInto(result, pathways, geneNames, container, W, zoom) {
  ...
  const tickSVG = ticks.map(v => {
    const y = _f(toY(v));
    return `
    <line x1="${M.l}" y1="${y}" x2="${M.l+pw}" y2="${y}"
...
// ── Interaction ───────────────────────────────────────────────
function _attachInteraction(container, result, pathways, geneNames) {
  const curve   = result.curve;
  const ord     = result.obsOrd;
  const nG      = curve.length;
  ...
  const barRows = sorted.map((r, i) => {
    ...
    return `
      <rect x="${padL}" y="${y}" width="${labelW - 4}" height="${barH}"
            fill="${fillBg}" rx="2"/>
      <text x="${_f(padL + labelW - 8)}" y="${_f(yMid)}"
            text-anchor="end" font-size="10.5" fill="#2a3040"
      ...
      <text x="${_f(valX)}" y="${_f(yMid)}"
            text-anchor="${valAnchor}" font-size="9.5" font-weight="500"
            fill="${fill}" font-family="'JetBrains Mono',monospace">
        ${_esc(nesStr)}
      </text>`;
  }).join('');
